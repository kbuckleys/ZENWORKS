...
Scope {
  ...
  function compileSettled() {
    ...
    // NO WINDOW IS BUILT HERE. Instantiating the tree costs 750ms on the
    // GUI thread even incubated, and a freeze five seconds after login is
    // worse than a slow login — you are using the machine by then. The
    // first window is built when something actually asks for one, which is
    // the moment that cost reads as "an application is opening".
    const q = mgr.pending;
    mgr.pending = [];
