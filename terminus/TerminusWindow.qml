...
FloatingWindow {
  ...
  function rewindViews() {
    root.actList.positionViewAtBeginning();
    root.midCol.view.positionViewAtBeginning();
    root.actGrid.positionViewAtBeginning();
  }
  ...
  Item {
    ...
    Keys.onPressed: (event) => {
      ...
      // SHIFT+RETURN OPENS A DIRECTORY IN A TAB OF ITS OWN — the keyboard's
      // version of the middle click that already does it. Before the plain
      // Return below, which takes any Return at all and opens the row in
      // place, modifiers and all. On a file there is no second place to put
      // it, so it opens the way Return always does.
      if ((event.modifiers & Qt.ShiftModifier)
          && (event.key === Qt.Key_Return || event.key === Qt.Key_Enter)) {
        event.accepted = true;
        const r = root.currentRow();
      ...
      // RIGHT AND l WALK THE TREE, and only that. They are the other half of
      // left and h, which go up; a key whose whole meaning is "go deeper"
      // should not also be able to launch a PDF in a reader. Over a file they
      // do nothing rather than something surprising, and Return is a key away.
      //
      // In grid the arrow has already moved the cursor sideways above — this
      // is the list and the columns, where there is no second axis for it to
      // mean, and `l` in every layout.
    ...
    Rectangle {
      ...
      clip: true
      z: 7
      // The same ground the breadcrumb and search strips stand on — but those
      // sit over the window's own background and this one sits over the ROWS,
      // so headBg's alpha alone left the listing legible straight through the
      // hints. An opaque floor under the same tint keeps the tone and stops
      // the text competing with a filename behind it.
    ...
    Rectangle {
      ...
      // but only just. At 0.72 a bright wallpaper came through the blur and up
      // between the rows, and a keymap you have to look PAST is not one you
      // can read at a glance. Dark enough that the keys carry the contrast and
      // the window behind is a suggestion of where you were.
      ...
      readonly property var groups: [
        ["move", [["j / k  ↓ ↑", "down / up"], ["h / l  ← →", "parent / enter"],
                  ...
                  [root.mouseKey(4), "up a directory"]]],
      ...
      Flickable {
        ...
        // ── AS MANY COLUMNS AS THERE IS ROOM FOR, up to three ────────
        // The keymap has outgrown one screen, and a single column on a wide
        // window is a narrow ribbon of text with two thirds of the card empty
        // beside it. Read DOWN each column and then across, which is how a
        // printed index reads and what keeps a section together instead of
        // scattering its lines across the width.
        //
        // 560px is the narrowest a column may be. It is set by the LONGEST
        // description rather than the average — "bookmark the item under the
        // cursor" beside a keycap — because a column that elides is a column
        // that has to be hovered to be read, which is the opposite of what a
        // keymap is for. A narrow window gets one column and never a cramped
        // two; three only appear on a window genuinely wide enough for them.
        Grid {
          id: helpGrid
          width: parent.width
          ...
          Repeater {
            ...
            delegate: Item {
              ...
              required property var modelData
              required property int index
              width: Math.floor(helpGrid.width / helpGrid.cols)
              height: 30
              ...
              // Named where it changes, and again at the top of a column it
              // runs into — a column that opens mid-section with no heading
              // is a list of keys belonging to nothing.
              readonly property bool starts: index === 0
                || index % helpGrid.perCol === 0
                || help.shownRows[index - 1][2] !== modelData[2]

              Rectangle {
                anchors.fill: parent
                // the same highlight the context menu and icarus use
                color: helpHov.hovered ? Zenon.headBg : "transparent"
              }
    ...
    Rectangle {
      ...
      // NO SCRIM. It was a modal card in the middle of the window and dimming
      // everything behind it was right for that. It is a dropdown off the path
      // bar now, and a dropdown does not dim the thing it drops out of — the
      // listing behind it is what you are choosing a destination relative to.
      // The shield underneath still catches a click outside, which is the only
      // part of being modal this needs to keep.
      color: "transparent"
      Behavior on opacity { NumberAnimation { duration: Zenon.normal; easing.type: Zenon.ease } }

      property bool open: false
      // The trail stands aside for it — see crumbFlick.
      readonly property bool inline: pathBar.open
      ...
      function dismiss() {
        ...
        pathBar.names = [];
        pathBar.namesText = "";
        // AND THE KEY THAT NAMES THEM. reload() skips the listing when the
        // directory being completed has not changed since last time — which
        // is right while the card is open and typing across one folder, and
        // catastrophic across a close: the candidates were thrown away above
        // and `lastDir` still claimed they were in hand, so the next open
        // completed against a list that no longer existed. No ghost, no rows,
        // nothing — and no way back, because every subsequent open matched the
        // same stale key. Clearing the data without clearing its key is what
        // made the card need a restart to work again.
        pathBar.lastDir = "";
        content.forceActiveFocus();
      }
      ...
      function accept() {
        // RETURN TAKES WHATEVER TAB WOULD HAVE TAKEN.
        //
        // A highlighted candidate is what Return means — you arrowed to it, so
        // going there is not a second decision. But neither is typing enough of
        // a name to identify it and pressing Return, which is how a path field
        // is ordinarily used: that submitted the half-typed fragment verbatim
        // and answered "no such path", which is true of the letters and has
        // nothing to do with what was meant. tabTarget already encodes the
        // rule — a lone candidate, or an ambiguity with nothing further in
        // common, is taken whole.
        if (pathBar.hits.length > 0) {
          const take = pathBar.tabTarget;
          if (take < 0) {
            // Several candidates with something still in common: take the
            // shared prefix, the way Tab would, and carry straight on. If that
            // now names a real directory then Return meant it, and asking for
            // a second press would be the card being pedantic about a decision
            // already made. If it does not, the check below says so honestly.
            pathBar.complete();
          } else {
            pathBar.setPath(
              Terminus.completedPath(pathBar.ctx.dir, pathBar.hits[take]));
            pathBar.pick = -1;
          }
        }
      ...
      // them wearing a shadow of its own was two answers to the same
      // question.
      MenuShadow { panel: pathCard; cornerRadius: Zenon.dialogRadius }

      ClippingRectangle {
        id: pathCard
        // ── OVER THE BREADCRUMBS, NOT OVER THE WINDOW ────────────────
        // It used to float in the middle of the screen like a launcher, which
        // is the wrong thing for it to be: you are editing the PATH, and the
        // path has a place on this window already. Sitting exactly where the
        // trail sits, the card reads as that trail becoming editable, and the
        // candidates hang off it the way a completion list should.
        //
        // Arithmetic rather than mapToItem: `chrome` is a Column filling the
        // window and the crumb bar is a direct child of it, so the bar's top
        // is the tab strip's height and its left is the trail's own x. Both
        // are ordinary property reads, which a binding can follow.
        x: crumbFlick.x
        y: tabStrip.height
        width: crumbFlick.width
        height: pathCol.implicitHeight
        color: Zenon.black
        border.color: Zenon.surfaceBorder
        border.width: 1
        radius: Zenon.dialogRadius
        // SQUARE WHERE IT MEETS THE BAR. The top edge is not an edge — it is
        // the seam with the strip the field is sitting in, and rounding it
        // drew two little notches of window either side of the path, which is
        // what made this read as a card lying on the bar rather than as the
        // bar opening.
        topLeftRadius: 0
        topRightRadius: 0
        ...
        Column {
          ...
          Item {
            width: parent.width
            // The trail's own height, so the field sits on the line the crumbs
            // sat on rather than near it.
            height: crumbInner.height

            // NO GLYPH. A folder icon in front of a path is a label on a
            // label: the field already begins with a slash, and the trail this
            // is standing in the place of never had one. It also pushed the
            // text a glyph's width to the right of where the first crumb sits,
            // so the bar jumped sideways as it opened.
            Item {
              id: pathGlyph
              anchors.left: parent.left
              anchors.verticalCenter: parent.verticalCenter
            ...
            TextInput {
              ...
              anchors.rightMargin: 14
              anchors.verticalCenter: parent.verticalCenter
              // Cyan, and so is the caret. What you are typing here is the one
              // live thing on the card — the ghost trailing it is dim and the
              // candidates below it are dimmer still — and it was the only one
              // of the three wearing the same white as a row of the listing
              // behind it.
              color: Zenon.cyan
              selectionColor: Zenon.selBg
              selectedTextColor: Zenon.white
              ...
              // The breathing caret the search bar and the keymap's search
              // already wear, which the note on both of those says the path
              // bar wears too — it did not. A cursorDelegate REPLACES the
              // built-in one, so there is exactly one and it is this.
              cursorDelegate: Rectangle {
                width: 2
                color: Zenon.cyan
                SequentialAnimation on opacity {
                  running: pathField.activeFocus
                  loops: Animation.Infinite
                  NumberAnimation { to: 0.2; duration: 620; easing.type: Easing.InOutQuad }
                  NumberAnimation { to: 1.0; duration: 620; easing.type: Easing.InOutQuad }
                }
        ...
        Column {
          ...
          ListView {
            ...
            delegate: Item {
              ...
              Rectangle {
                anchors.fill: parent
                color: appRow.target ? Zenon.selBg
                  : (appHov.hovered ? Zenon.hoverTint : "transparent")
              }
  ...
  component Pane: QtObject {
    ...
    function syncView() {
      ...
      if (m.count === 0 || same < n * 0.5) {
        // BEFORE the rows change, never after — see rewindViews.
        //
  ...
  component PrefAction: Item {
    ...
    Rectangle {
      anchors.fill: parent
      color: act.enabled && actHov.hovered ? Zenon.headBg : "transparent"
    }
  ...
  component PrefRow: Item {
    ...
    Rectangle {
      ...
      color: prefHov.hovered ? Zenon.headBg : "transparent"
    }
