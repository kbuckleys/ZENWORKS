...
function home() {
  return Quickshell.env("HOME") || "/home/buck";
}
...
function getApps(modelValues) {
  ...
  for (const v of modelValues) {
    ...
    seen[key] = true;
    apps.push({ name, exec, id, icon: v.icon || "" });
  }
...
function terminalCommand(cmd) {
  return "setsid kitty -1 --detach --hold --title runner " + shell() +
      " -i -c " + shellQuote(cmd) + " >/dev/null 2>&1";
}
...
function highlight(text, query) {
  ...
  return escMarkup(t.slice(0, idx)) + "<b>" + escMarkup(t.slice(idx, idx + q.length)) +
      "</b>" + escMarkup(t.slice(idx + q.length));
}
