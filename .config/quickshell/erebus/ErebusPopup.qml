...
PanelWindow {
  ...
  Item {
    ...
    // pillWidth, not width — see the note on cynosure's liveWidth. The
    // bar's surface is no longer the pill.
    width: (popup.morphMode && popup.statusbar && popup.statusbar.pillWidth > 0)
      ? popup.statusbar.pillWidth : Zenon.layerWidth(250)
    ...
    height: (popup.morphMode && popup.statusbar && popup.statusbar.pillHeight > 0)
      ? popup.statusbar.pillHeight : popup.calcHeight()
