...
LayerPopup {
  ...
  // widest credential row — folder › user · name, as drawn — times the
  // columns showing, with the hint strip of the current view as the floor,
  // and the 1000 every layer shares as the ceiling. The actions view is
  // about one entry, so there it is that entry's name that has to fit.
  FontMetrics { id: rowFm; font.family: Zenon.face; font.pixelSize: 16 }
  FontMetrics { id: rowBoldFm; font.family: Zenon.face; font.bold: true; font.pixelSize: 16 }
  function rowTextW(e) {
    if (!e) return 0;
    return (e.folder ? rowFm.advanceWidth(e.folder + "  ›  ") : 0)
      + (e.user ? rowFm.advanceWidth(e.user + "  ·  ") : 0)
      + rowBoldFm.advanceWidth(String(e.name || ""));
  }
  ...
  readonly property int panelWidth: {
    if (popup.mode === "actions" && popup.activeEntry)
      return Math.max(popup.minW, Math.min(1000,
        Math.ceil(rowBoldFm.advanceWidth(String(popup.activeEntry.name || "")) + 120)));
    if (popup.mode !== "list") return popup.minW;
    let most = 0;
    for (const e of popup.filtered) most = Math.max(most, popup.rowTextW(e));
    // the row's own 10 + 8 insets, and a few pixels for shaping
    const cell = Math.ceil(most) + 24;
        ...
        Column {
          ...
          GridView {
            ...
            delegate: Item {
              ...
              Rectangle {
                anchors.fill: parent
                color: modelData === popup.sel ? Zenon.border : "transparent"
              }
          ...
          Item {
            ...
            Item {
              ...
              CapsGlyph {
                ...
                y: fieldRow.y + (fieldRow.height - height) / 2
              }
