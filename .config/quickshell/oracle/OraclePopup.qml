...
PanelWindow {
    ...
    Rectangle {
      ...
      MouseArea {
        ...
        onClicked: {
          ...
          const o = Zenon.winOrigin(popup, popup.screen);
          const p = dvChip.mapToItem(null, 0, dvChip.height + 4);
          popup.openEnumMenu(dv.spec.key, Qt.point(o.x + p.x, o.y + p.y));
        }
      }
    }
  }
  ...
  component InfoControl: Item {
    ...
    Text {
      ...
      text: nfo.value
      // A Health row that found something says so in colour; every other
      // fact stays dim.
      color: {
            ...
            Column {
              ...
              Repeater {
                ...
                delegate: Item {
                  ...
                  Rectangle {
                    anchors.fill: parent
                    color: sectionRow.here ? Zenon.border : "transparent"
                    Behavior on color { ColorAnimation { duration: Zenon.fast; easing.type: Zenon.ease } }
                  }
  ...
  CardMenu {
    ...
    // the narrowest it may be; it grows to its longest option rather than
