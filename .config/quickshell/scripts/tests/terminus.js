...
module.exports = {
  ...
  cases: (T, t) => {
    ...
    // ── a folder's facts, for quick look ─────────────────────────────────
    // The shape the command prints: stat's owner, du's total, two wc counts,
    // then the marker. A walk killed by stepping on stops before the marker.
    const ff = T.parseFolderFacts("buck:buck\n4096123\n41\n7\nend\n");
    t.eq("owner is read", ff.owner, "buck:buck");
    t.eq("size is read", ff.size, 4096123);
    t.eq("files are read", ff.files, 41);
    t.eq("dirs are read", ff.dirs, 7);
    t.eq("a killed walk is not believed",
      T.parseFolderFacts("buck:buck\n4096123\n12\n"), null);
    t.eq("nothing at all is not believed", T.parseFolderFacts(""), null);
    // ── archives, walked into ────────────────────────────────────────────
    const MNT = { "/run/u/terminus/archives/abc": "/home/b/Down loads/pack.zip" };
    t.eq("the top of a mount shows as the archive",
      T.shownPath("/run/u/terminus/archives/abc", MNT), "/home/b/Down loads/pack.zip");
    t.eq("and inside it, with the rest",
      T.shownPath("/run/u/terminus/archives/abc/in dir", MNT), "/home/b/Down loads/pack.zip/in dir");
    t.eq("a sibling with the same prefix is not inside",
      T.mountOf("/run/u/terminus/archives/abcd", MNT), null);
    t.eq("up from the top is the archive's folder",
      T.parentOf("/run/u/terminus/archives/abc", MNT), "/home/b/Down loads");
    t.eq("up from inside is ordinary",
      T.parentOf("/run/u/terminus/archives/abc/in dir", MNT), "/run/u/terminus/archives/abc");
    const ac = T.archiveCrumbs("/run/u/terminus/archives/abc/in dir", "/home/b", MNT);
    t.eq("crumbs read as the archive's path",
      ac.map((c) => c.label).join("/"), "~/Down loads/pack.zip/in dir");
    t.eq("the archive's crumb leads into the mount", ac[2].path, "/run/u/terminus/archives/abc");
    t.eq("and the folder's crumb to the real folder", ac[1].path, "/home/b/Down loads");
    t.ok("mounting quotes both paths",
      T.mountArchiveCommand("/a b.zip", "/m n", "/i").indexOf("-- '/a b.zip' '/m n'") >= 0);
    t.ok("unmounting is lazy", T.unmountArchiveCommand("/m").indexOf("-uz") >= 0);

    // ── the pdf pages ─────────────────────────────────────────────────────
    t.eq("a page count is read", JSON.stringify(T.parsePdfLine("pages 12")),
      JSON.stringify({ kind: "pages", n: 12 }));
    t.eq("a thumbnail is read", T.parsePdfLine("thumb 3\n").n, 3);
    t.eq("pdftoppm noise is not", T.parsePdfLine("Syntax Error: xref"), null);
    const pc = T.pdfPagesCommand("/tmp/a b.pdf", "/c/pdf/k", 160);
    t.ok("the path is quoted", pc.indexOf("'/tmp/a b.pdf'") >= 0);
    t.ok("only its own cache is swept", pc.indexOf("find '/c/pdf' -mindepth 1") >= 0);
    t.ok("one page renders only that page",
      T.pdfPageCommand("/x.pdf", "/c/k", 4, 1600).indexOf("-f 4 -l 4") >= 0);
    t.eq("the tag comes back",
      T.parseFolderFacts("u:g\n1\n2\n3\nend 17\n").tag, "17");
    t.eq("an unreadable owner is empty, not a shift",
      T.parseFolderFacts("\n0\n0\n0\nend\n").owner, "");
    t.ok("the command quotes a path with spaces",
      T.folderFactsCommand("/tmp/testing again").indexOf("'/tmp/testing again'") >= 0);

    // ── the split's room ──────────────────────────────────────────────────
    const mons = [
      { id: 0, x: 0, y: 0, width: 1920, height: 1080, scale: 1, transform: 3, reserved: [0, 0, 0, 38] },
      { id: 1, x: 1080, y: 0, width: 2560, height: 1440, scale: 1, transform: 0, reserved: [0, 0, 0, 0] }];
    const fl = (x, w, mon) => ({ floating: true, fullscreen: 0, monitor: mon, at: [x, 100], size: [w, 600] });
    t.eq("a split grows by what it asks",
      T.splitRoom(fl(1800, 1000, 1), mons, 1800, 16, 560).dw, 800);
    t.eq("never past the monitor",
      T.splitRoom(fl(1100, 1000, 1), mons, 3000, 16, 560).dw, 1528);
    t.eq("a rotated monitor is as narrow as it looks",
      T.splitRoom(fl(100, 800, 0), mons, 1600, 16, 560).dw, 1080 - 32 - 800);
    t.eq("tiled is the layout's business", T.splitRoom(Object.assign(fl(0, 800, 1), { floating: false }), mons, 1600, 16, 560), null);
    t.eq("so is fullscreen or maximized", T.splitRoom(Object.assign(fl(0, 800, 1), { fullscreen: 1 }), mons, 1600, 16, 560), null);
    t.eq("a hand-widened window is not shrunk by a grow",
      T.splitRoom(fl(1090, 1200, 0), mons, 2000, 16, 560), null);
    t.eq("unsplitting hands the room back",
      T.splitRoom(fl(1400, 1800, 1), mons, 1000, 16, 560).dw, -800);
