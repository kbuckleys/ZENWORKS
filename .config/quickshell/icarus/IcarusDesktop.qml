...
Variants {
  ...
  // Any press on the desktop, whatever the button — shell.qml passes it to
  // whatever needs to know that you clicked away (terminus' menus).
  ...
  PanelWindow {
    ...
    Item {
      ...
      // Passive: it takes no grab, so the right-click menu below still gets
      // its press exactly as before.
      PointHandler {
        acceptedButtons: Qt.AllButtons
        onActiveChanged: if (active) root.desktopPressed()
      }
