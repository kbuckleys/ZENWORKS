...
Scope {
  ...
  readonly property var listeners: {
    ...
    //
    // Every device oracle names now, not one keyboard — see idleLightDevices
    // there, and "auto" resolved against what solaar reported. None, no
    // listener, so a machine without solaar never spawns anything.
    //
    // AND BACK TO WHERE IT WAS, not to 100. Hypridle's line forced full
    // brightness on every return, which undid whatever level you had set.
    const lights = Oracle.lightDevices();
    if (lights.length > 0 && Oracle.idleKeyboardSecs > 0) out.push({
      timeout: Oracle.idleKeyboardSecs,
      onTimeout: () => root.lights(lights, false),
      onResume: () => root.lights(lights, true)
    });
  ...
  // ── DIMMING, AND REMEMBERING WHAT IT WAS ─────────────────────────────
  // The level each device had is read and kept in the runtime dir before it
  // goes to 0, and put back on resume. Kept only when above 0: an idle that
  // fires twice without a resume between would otherwise remember "off" as
  // the level to come back to. Nothing kept means 100, hypridle's answer.
  //
  // A shell this time, because reading a value and writing it to a file is
  // a pipeline — but the names still arrive as ARGUMENTS ("$@"), never
  // spliced into the script, for the reason run() gives.
  function lights(names, on) {
    const script = 'dir="${XDG_RUNTIME_DIR:-/tmp}/socordia-lights"; mkdir -p "$dir"; '
      + 'for d in "$@"; do '
      + 'f="$dir/$(printf %s "$d" | md5sum | cut -c1-16)"; '
      + (on
        // ── UNTIL IT ANSWERS ─────────────────────────────────────────
        // The keypress that ends an idle is very often the one that WAKES
        // the keyboard — its own firmware sleeps it after five minutes,
        // the same as the dim — and a command sent while it is still
        // coming back is lost. One try left the backlight dark for good.
        // So it is set, read back, and tried again a second later, until
        // the device reports the level or ten seconds have gone by.
        ? 'v=$(cat "$f" 2>/dev/null); case "$v" in ""|*[!0-9]*) v=100 ;; esac; '
          + 'n=0; while [ $n -lt 10 ]; do '
          + 'solaar config "$d" brightness_control "$v" >/dev/null 2>&1; '
          + 'now=$(solaar config "$d" brightness_control 2>/dev/null '
          + '| sed -n "s/^brightness_control = //p"); '
          + '[ "$now" = "$v" ] && break; n=$((n+1)); sleep 1; done & '
        : 'v=$(solaar config "$d" brightness_control 2>/dev/null '
          + '| sed -n "s/^brightness_control = //p"); '
          + 'case "$v" in ""|0|*[!0-9]*) ;; *) printf %s "$v" > "$f" ;; esac; '
          + 'solaar config "$d" brightness_control 0 >/dev/null 2>&1; ')
      + 'done';
    // One device's retries must not hold up the next one's — hence the `&`
    // above — and the shell waits for them all so nothing is orphaned.
    root.run(["sh", "-c", script + "; wait", "socordia-lights"].concat(names));
    root.lightsDimmed = !on;
    root.dimmedNames = on ? [] : names;
  }

  // ── A SAFETY NET FOR THE WAY BACK ────────────────────────────────────
  // What was dimmed, so that the end of ANY idle brings it back — not only
  // the end of the listener that dimmed it. That listener is one delegate
  // of an Instantiator over a computed list, and a delegate rebuilt while
  // you were away is a delegate that never sees you return.
  property bool lightsDimmed: false
  property var dimmedNames: []
  ...
  Instantiator {
    ...
    delegate: IdleMonitor {
      ...
      onIsIdleChanged: {
        if (isIdle) { modelData.onTimeout(); return; }
        if (modelData.onResume) modelData.onResume();
        // any return from idle, if the lights were left down — see
        // lightsDimmed
        if (root.lightsDimmed) root.lights(root.dimmedNames, true);
      }
    }
  }
  ...
  IpcHandler {
    ...
    function awake(): string {
      ...
      // Said out loud, because a toggle you cannot see is a toggle you
      // forget is on — and then the screen simply never locks.
      Quickshell.execDetached(["notify-send", "-a", "socordia",
        root.forceAwake ? "Keep Awake is on" : "Keep Awake is off",
        root.forceAwake ? "The screen will not lock, dim or turn off until you switch this off."
                        : "Idle lock and screen-off are back on their timers."]);
