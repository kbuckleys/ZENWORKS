...
PanelWindow {
  ...
  readonly property color selColor: "#4d2d505c"
  readonly property color msgColor: "#66282f36"
  readonly property color msgBorder: "#4d2d505c"
  ...
  readonly property int imgPerRow: Math.max(1, Math.floor(popup.implicitWidth / popup.imgCell))
  ...
  function cellText(preview) {
    const cellW = popup.textColsUsed === 1 ? popup.implicitWidth : textGrid.cellWidth;
    ...
    return Cliphist.highlightedPreview(p, popup.query);
  }
  ...
  function log(msg) {
    logProc.command = ["sh", "-c",
      "printf '%s\\n' " + Cliphist.shellQuote(Qt.formatTime(new Date(), "hh:mm:ss") + " " + msg) +
      " >> /tmp/cliphist_state.txt"];
  ...
  function onList(text) {
    const parsed = Cliphist.parseList(text);
  ...
  function applyFilter() {
    popup.rows = Cliphist.filterEntries(popup.entries, popup.query);
    popup.clampSel();
  }

  function generateThumbs() {
    if (thumbProc.running) return;
    const cmd = Cliphist.thumbCommand(popup.imageIds, Cliphist.thumbDir());
    if (!cmd) return;
...
function onThumbsDone() {
  ...
  function confirm() {
    ...
    popup.runAction(Cliphist.copyCommand(id), "copy");
  }

  function openSelected() {
    if (popup.mode !== "image" || popup.imgRows.length === 0) return;
    popup.runAction(Cliphist.openCommand(popup.imgRows[popup.sel].id, Cliphist.openDir()), "open");
  }
  ...
  function deleteSelected() {
    ...
    const id = popup.mode === "image"
      ? popup.imgRows[popup.sel].id
      : popup.rows[popup.sel].id;
    popup.runAction(Cliphist.deleteCommand(id, Cliphist.thumbDir()), "delete");
  }
  ...
  function moveVert(delta) {
    const stride = popup.mode === "image" ? popup.imgPerRow : popup.textColsUsed;
    popup.stepSel(delta * stride);
  }
    ...
    Rectangle {
      ...
      Rectangle {
        ...
        Row {
          ...
          Repeater {
            model: Cliphist.hintText(popup.mode)
            Text {
              required property var modelData
              text: modelData
              color: popup.hintColor
