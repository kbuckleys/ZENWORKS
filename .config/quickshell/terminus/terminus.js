...
// ── AN ARCHIVE, WALKED INTO ─────────────────────────────────────────────
// Opening an archive mounts it READ-ONLY with ratarmount under the runtime
// directory, and the window simply walks into the mount. From there it is a
// directory like any other — every view, the preview, quick look, search and
// copying out all work unchanged, because nothing is being faked.
//
// The one thing a mount gets wrong is its NAME: it lives at a hashed path in
// /run/user, and that is not where the archive is. `mounts` maps each mount
// point to the archive it is, and these few functions are the whole of the
// translation — what to SHOW for a directory, where UP goes, and what the
// crumbs are, with every crumb still leading somewhere real.

// The mount a directory is inside, as { mnt, archive }, or null.
function mountOf(dir, mounts) {
  const d = String(dir || "");
  for (const m in (mounts || {}))
    if (d === m || d.indexOf(m + "/") === 0) return { mnt: m, archive: mounts[m] };
  return null;
}

// The path to show for a directory: the archive's own, with the part inside.
function shownPath(dir, mounts) {
  const at = mountOf(dir, mounts);
  return at ? at.archive + String(dir).slice(at.mnt.length) : String(dir);
}

// Where up goes. Out of an archive's top is the folder the archive sits in.
function parentOf(dir, mounts) {
  const at = mountOf(dir, mounts);
  if (at && String(dir) === at.mnt) return dirname(at.archive);
  return dirname(dir);
}

// The crumbs, spelled as the archive's path, each pointing at a directory
// that exists — so the archive's crumb is its mount and the ones inside it
// are inside the mount.
function archiveCrumbs(dir, home, mounts) {
  const at = mountOf(dir, mounts);
  if (!at) return crumbs(dir, home);
  return crumbs(shownPath(dir, mounts), home).map((c) => {
    if (c.path === at.archive || c.path.indexOf(at.archive + "/") === 0)
      return { label: c.label, path: at.mnt + c.path.slice(at.archive.length) };
    return c;
  });
}

// Mount it, unless it already is; exit 0 only when it really is mounted. The
// index ratarmount builds for a big archive goes into our cache, never beside
// the archive in your folder.
function mountArchiveCommand(archive, mnt, indexDir) {
  const m = Strings.shellQuote(mnt);
  return "mkdir -p " + m + " " + Strings.shellQuote(indexDir) + " || exit 1; "
    + "mountpoint -q " + m + " || ratarmount --index-folders "
    + Strings.shellQuote(indexDir) + " -- " + Strings.shellQuote(archive) + " " + m
    + " >/dev/null 2>&1; mountpoint -q " + m;
}

// Lazy, so a file still open from inside it (a film playing) keeps working
// until it is closed, instead of the unmount failing and the mount lingering.
function unmountArchiveCommand(mnt) {
  const m = Strings.shellQuote(mnt);
  return "fusermount3 -uz -- " + m + " 2>/dev/null; rmdir -- " + m + " 2>/dev/null; true";
}

// Whatever a previous run left mounted — a crash, a restart mid-browse.
function sweepArchivesCommand(base) {
  return "for m in " + Strings.shellQuote(base) + "/*; do [ -d \"$m\" ] || continue; "
    + "fusermount3 -uz -- \"$m\" 2>/dev/null; rmdir -- \"$m\" 2>/dev/null; done; true";
}
...
// ── EVERY PAGE, FOR QUICK LOOK'S SIDEBAR ─────────────────────────────────
// pdfCommand renders page one for the pane. Quick look wants the whole
// document: how many pages there are, a small picture of each for the strip
// down its side, and a sharp render of whichever page you are reading.
//
// The small ones are made ONE PAGE AT A TIME, each announced as it lands
// ("thumb N"), so the strip fills in while you read rather than after the
// last page of a long document. pdftoppm renders a whole range in one go,
// but it pads its output names by the page count and says nothing until it
// is done; one call per page is a little more work for a strip that is
// never empty. Already-made pages are skipped, so a document looked at
// before costs one pdfinfo.
//
// Cached per VERSION of the file — `dir` is named by the caller from path,
// size and mtime — and touched on use, with anything unvisited for a fortnight
// swept on the way in, so the pages of documents long gone do not pile up.
function pdfPagesCommand(path, dir, thumbPx) {
  const q = Strings.shellQuote(path);
  const d = Strings.shellQuote(dir);
  const root = Strings.shellQuote(dirname(dir));
  const px = Math.max(40, Math.round(Number(thumbPx) || 160));
  return "mkdir -p " + d + " && touch " + d + "; "
    + "find " + root + " -mindepth 1 -maxdepth 1 -type d -mtime +14 "
    + "-exec rm -rf {} + 2>/dev/null; "
    + "n=$(pdfinfo -- " + q + " 2>/dev/null | sed -n 's/^Pages:[[:space:]]*//p'); "
    + "case \"$n\" in ''|*[!0-9]*) n=0 ;; esac; "
    + "echo \"pages $n\"; "
    + "i=1; while [ \"$i\" -le \"$n\" ]; do "
    + "[ -s " + d + "/t$i.png ] || pdftoppm -png -f $i -l $i -singlefile "
    + "-scale-to " + px + " -- " + q + " " + d + "/t$i 2>/dev/null; "
    + "[ -s " + d + "/t$i.png ] && echo \"thumb $i\"; "
    + "i=$((i+1)); done";
}

// One page, sharp, at the size quick look will draw it. "page N" once it
// is on disk; nothing if it could not be made.
function pdfPageCommand(path, dir, page, px) {
  const p = Math.max(1, Math.round(Number(page) || 1));
  const f = Strings.shellQuote(dir + "/p" + p + ".png");
  return "[ -s " + f + " ] || pdftoppm -png -f " + p + " -l " + p
    + " -singlefile -scale-to " + Math.max(200, Math.round(Number(px) || 1600))
    + " -- " + Strings.shellQuote(path) + " " + Strings.shellQuote(dir + "/p" + p)
    + " 2>/dev/null; [ -s " + f + " ] && echo \"page " + p + "\"";
}

// A line of either command's output, as { kind, n } — or null for anything
// else pdftoppm might have said.
function parsePdfLine(line) {
  const m = /^(pages|thumb|page) (\d+)$/.exec(String(line || "").trim());
...
// Quick look's folder card shows the same facts the sheet does, and three of
// them cost a process each there: owner, total size and the recursive count.
// Here they are one shell, so stepping through folders with space held is one
// spawn per folder rather than three.
//
// ENDS WITH A MARKER. Stepping on kills a walk still in flight, and a killed
// `du` prints nothing while a killed `find | wc` prints a partial count that
// looks exactly like a real one. Only output that reached the marker is
// believed — see parseFolderFacts.
//
// AND THE MARKER CARRIES A TAG. A walk that finishes on its own a moment
...
// owner, bytes, files, dirs, tag — or null for a run that did not finish.
function parseFolderFacts(text) {
  const l = String(text || "").split("\n");
  if (l.length < 5) return null;
  const m = /^end ?(.*)$/.exec(l[4].trim());
  if (!m) return null;
  const n = (s) => { const v = parseInt(String(s).trim(), 10); return isNaN(v) ? -1 : v; };
  return { owner: l[0].trim(), size: n(l[1]), files: n(l[2]), dirs: n(l[3]), tag: m[1] };
}
...
function appsCommand(path) {
  const q = Strings.shellQuote(path);
  // ── GIO'S TYPE, BECAUSE GIO OPENS IT ─────────────────────────────────
  // This asked xdg-mime, which sniffs the CONTENTS — and a .webm is a
  // Matroska container, so it came back video/x-matroska, whose default was
  // mpv. `gio open`, which is what actually runs on a double click, goes by
  // video/webm, whose default was firefox. The card said mpv and firefox
  // opened, and choosing a default here set it for a type the launcher
  // never asks about. Asking the launcher's own question makes the label,
  // the list and the double click all the same answer. xdg-mime only when
  // gio has none to give.
  return "m=$(gio info -a standard::content-type -- " + q
    + " 2>/dev/null | sed -n 's/^ *standard::content-type: //p'); "
    + "[ -n \"$m\" ] || m=$(xdg-mime query filetype " + q + " 2>/dev/null); "
    + "[ -n \"$m\" ] || exit 0; "
...
// ── THE SPLIT, AND THE ROOM IT NEEDS ──────────────────────────────────────
// Splitting a floating window asks for more room; unsplitting hands it back.
// Quickshell cannot resize a mapped window (implicitWidth is read once, at
// birth), so hyprland does it: these work out the numbers from its own json.
//
// `win` is `hyprctl -j activewindow`, `mons` is `hyprctl -j monitors`. The
// answer is { dw }, a relative resize no wider than the monitor's usable
// area — or null when there is nothing to do: tiled, fullscreen or maximized
// windows are the layout's to size, not ours. Hyprland then centres it.
//
// A monitor's width and height are its mode, before rotation and scale; its
// `reserved` is [left, top, right, bottom], the bar's share of it.
function splitRoom(win, mons, wantW, margin, minW) {
  if (!win || !win.floating || win.fullscreen || !win.size) return null;
  const mon = (mons || []).find((m) => m.id === win.monitor);
  if (!mon) return null;
  const turned = (mon.transform || 0) % 2 === 1;
  const scale = mon.scale > 0 ? mon.scale : 1;
  const lw = (turned ? mon.height : mon.width) / scale;
  const r = mon.reserved || [0, 0, 0, 0];
  const left = mon.x + r[0] + margin;
  const right = mon.x + lw - r[2] - margin;
  const w = win.size[0];
  // Never past the monitor when growing — but a window you already made
  // wider than that by hand is not shrunk by a grow either.
  let newW = Math.min(wantW, Math.max(right - left, w));
  newW = Math.round(Math.max(newW, minW || 0));
  if (newW === w) return null;
  return { dw: newW - w };
}

function splitRoomDispatches(address, room) {
  const sel = 'window = "address:' + address + '"';
  return ["hl.dsp.window.resize({ x = " + room.dw + ", y = 0, relative = true, " + sel + " })",
    "hl.dsp.window.center({ " + sel + " })"];
}
