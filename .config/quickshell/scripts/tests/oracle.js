...
const BOOL = { key: "showCpu", section: "bar", label: "CPU meter", type: "bool",
               fallback: true };
const DEVS = { key: "idleLightDevices", section: "idle", label: "Dim lighting on",
               type: "devices", fallback: "auto" };
...
module.exports = {
  ...
  cases: (T, t) => {
    ...
    // ── the devices setting ─────────────────────────────────────────────
    t.eq("auto stays auto", T.coerce(DEVS, " auto "), "auto");
    t.eq("names are trimmed and de-duplicated",
      T.coerce(DEVS, " G515 TKL \n\nG502\nG515 TKL"), "G515 TKL\nG502");
    t.eq("none is empty", T.coerce(DEVS, null), "");

    // Real `solaar show` shapes: a wired keyboard at the margin, and a
    // receiver with a mouse paired under it. Trimmed, but the lines kept.
    const SHOW = [
    ...
    const found = T.parseSolaarShow(SHOW);
    t.eq("three blocks: keyboard, receiver, paired mouse",
      found.map((d) => d.name).join("|"), "G515 TKL|Lightspeed Receiver|G502 X PLUS");
    t.ok("a firmware line is not a paired device",
      found.every((d) => d.name !== "BL2 32.00.B0006"));
    t.eq("the keyboard can be dimmed", found[0].dimmable, true);
    t.eq("and knows what it is", found[0].kind, "keyboard");
    t.eq("the mouse cannot", found[2].dimmable, false);
    t.eq("auto means the first dimmable one",
      T.lightDevices("auto", found).join("|"), "G515 TKL");
    t.eq("auto with nothing dimmable is nothing",
      T.lightDevices("auto", [found[1]]).length, 0);
    t.eq("an explicit list is taken as it is",
      T.lightDevices("A\nB", found).join("|"), "A|B");
    t.eq("and none is none", T.lightDevices("", found).length, 0);

    // the dropdown
    const rowsAuto = T.deviceRows("auto", found, false);
    t.eq("automatic is first and ticked", rowsAuto[0].mark, true);
    t.eq("only the dimmable device is offered",
      rowsAuto.filter((r) => r.act === "toggle").map((r) => r.name).join("|"), "G515 TKL");
    t.eq("and it is ticked under auto",
      rowsAuto.filter((r) => r.act === "toggle")[0].mark, true);
    t.eq("rescan is last", rowsAuto[rowsAuto.length - 1].act, "rescan");
    t.eq("a chosen device that is unplugged is still listed",
      T.deviceRows("Old Mouse", found, false)
        .filter((r) => r.name === "Old Mouse").length, 1);
    t.eq("while scanning the last row cannot be clicked",
      T.deviceRows("auto", found, true).slice(-1)[0].enabled, false);
    t.eq("unticking the auto choice leaves none",
      T.toggleDevice("auto", "G515 TKL", found), "");
    t.eq("ticking adds to what was there",
      T.toggleDevice("G515 TKL", "G502 X PLUS", found), "G515 TKL\nG502 X PLUS");
    t.eq("the chip under auto names the device",
      T.displayDevices("auto", found, true), "Auto \u00b7 G515 TKL");
    t.eq("and says when there is nothing", T.displayDevices("auto", [], true),
      "Auto \u00b7 none found");
    t.eq("two chosen reads as a count", T.displayDevices("A\nB", found, true), "2 devices");
    t.eq("none reads as none", T.displayDevices("", found, true), "None");

    // ── the checkup ───────────────────────────────────────────────────────
    const good = "tests\t920 checks pass\ntestfail\t\nprobes\t\nstale\t\nrss\t1054396\n"
      + "held\tinotifywait,gdbus\nspawned\tgpu.sh\nerrors\t0\nwarnings\t1\nlasterror\t\n";
    const G = T.readCheckup(good, 1054396);
    t.eq("a clean run is all ok",
      Object.keys(G).filter((k) => G[k].level !== "ok").length, 0);
    t.eq("memory reads against the baseline", G.memory.text, "1.05 GB \u00b7 baseline 1.05 GB (+0%)");
    t.eq("the log counts both", G.log.text, "0 errors \u00b7 1 warning");
    const B = T.readCheckup("tests\t3 of 920 checks FAILED\ntestfail\tceres,terminus\n"
      + "probes\tcerberus/Cerberus Lock.qml\nstale\ta.js,b.js\nrss\t2000000\n"
      + "spawned\thyprctl,hyprctl,hyprctl,hyprctl,hyprctl\nerrors\t2\n", 1054396);
    t.eq("a failing suite is named", B.tests.text, "3 of 920 checks FAILED \u2014 ceres, terminus");
    t.eq("and is bad", B.tests.level, "bad");
    t.eq("a probe names its file", B.probes.text, "1 left in cerberus/Cerberus Lock.qml");
    t.eq("changed scripts ask for a restart", B.stale.level, "warn");
    t.eq("nearly double the baseline is bad", B.memory.level, "bad");
    t.eq("a burst of spawns is a warning", B.pollers.level, "warn");
    t.has("repeats are counted, not listed", B.pollers.text, "hyprctl \u00d75");
    t.eq("errors are bad", B.log.level, "bad");
    t.eq("no output at all did not run", T.readCheckup("", 1).tests.text, "did not run");
  }
};
