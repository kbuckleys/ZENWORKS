...
MenuShadow {
  ...
  //
  // ── THE EASE IS ON THE GATE, NEVER ON THE OPACITY ────────────────────
  // It used to be `Behavior on opacity`, and cynosure bound `opacity` to its
  // showFactor so the shadow would fade in with the panel. A Behavior
  // intercepts EVERY change — and showFactor changes every frame of the
  // entrance, so each frame restarted the shadow's own ease at t=0, where
  // its value is its starting value. The shadow sat frozen at nothing for
  // the whole of the panel's fade and only began once the panel had
  // stopped: ten frames late at 60Hz, measured off a recording.
  //
  // So the two are kept apart. `morphed` is a switch and gets the ease;
  // `fade` is already a curve — the caller's showFactor, the bar's intro —
  // and is followed exactly, frame for frame.
  property real fade: 1
  property real gate: root.morphed ? 0 : 1
  Behavior on gate { NumberAnimation { duration: Zenon.normal; easing.type: Zenon.ease } }
  opacity: root.gate * root.fade
