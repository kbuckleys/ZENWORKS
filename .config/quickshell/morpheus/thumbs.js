...
function generate(jobs, big) {
  ...
  return "mkdir -p " + d + "; printf '%s\n' " + Strings.shellQuote(args)
    ...
    + keyScript()
    + 'out=' + d + '/$key' + tail + '.' + EXT + '; '
    // ── A THUMBNAIL IN USE IS TOUCHED ──────────────────────────────
    // sweep() deletes by age, on the reasoning that anything alive gets
    // regenerated. Nothing alive ever was: an existing thumbnail was
    // reused untouched, so a month after it was made the sweep at the end
    // of a scan deleted it straight after the scan had reported it, and
    // the picker drew a blank tile. Every file in the pool dates from the
    // same two days, so they would all have gone at once. Touched on use,
    // age means "nobody has asked for this in a month", as it was meant to.
