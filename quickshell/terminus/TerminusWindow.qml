...
FloatingWindow {
  ...
  // "wallpaper set" and "3 to copy" arrived in the colour of a failure and
  ...
  function keepScroll() {
    return { list: list.contentY, mid: midList.contentY,
             parent: parentList.contentY, grid: grid.contentY };
  }

  function restoreScroll(keep) {
    if (!keep) return;
    root.putScroll(list, keep.list);
    root.putScroll(midList, keep.mid);
    root.putScroll(parentList, keep.parent);
    root.putScroll(grid, keep.grid);
  }
  ...
  // `view` is an array, and handing a view a new array is a full reset: every
  // delegate destroyed and rebuilt, the scroll offset dropped, hover lost,
  // thumbnails re-decoded. In a directory something else is writing to — /tmp,
  // a download folder, a build tree — that happens every second or two, and no
  // amount of putting the scroll back afterwards stops the churn underneath.
  //
  // So the views bind to a MODEL that is edited rather than replaced, and it
  // carries one role: the path. Identity only. Everything a row draws with is
  // looked up from `viewIndex` by that path, so when a file's size or mtime
  // changes the delegate's own binding re-evaluates and the row updates in
  // place — no destruction, nothing to put back.
  //
  // `view` stays exactly as it was, because fifty other things read it as an
  // array: the cursor is an index into it, the rubber band turns pixels into
  // indices, the sort and the filter produce it. This is a second view OF the
  // same list, not a second copy of it.
  ListModel { id: viewModel }

  // path -> row, rebuilt with the listing. Replaced wholesale on purpose: that
  // is one binding re-evaluation per visible delegate, which is a map lookup
  // each, against destroying and rebuilding every one of them.
  // NOT a binding. A binding is evaluated when it is first read, and the model
  // is synced before anything reads this — so a delegate created for a path
  // that had only just been appended looked it up in the map as it was BEFORE
  // the listing changed, and got nothing. Written eagerly, in the same handler
  // that syncs the model and before it, the two can never disagree.
  property var viewIndex: ({})

  function rowFor(path) {
    return root.viewIndex["k:" + String(path)] || null;
  }
  ...
  // The diff. Removals first so the forward walk never has to step over a row
  // that is on its way out, then one pass placing what is left.
  function syncView() {
    const next = root.view;
    const m = viewModel;

    // A WHOLESALE CHANGE IS CHEAPER TO REBUILD than to walk into place, and
    // there are two of them: arriving in a different directory, and re-sorting
    // the one you are in. Both move nearly every row, and the walk below is
    // quadratic when nearly every row has moved — on a four thousand entry
    // directory that is the difference between instant and a visible stall.
    // Their scroll is meant to reset anyway.
      ...
      onStreamFinished: {
        ...
        if (root.wantSel !== "") {
          ...
          for (let i = 0; i < root.view.length; ++i) {
            if (root.view[i].path === want) {
              root.sel = i; root.anchor = i; land = true; break;
            }
          }
          ...
          // the same file, wherever it ended up
          for (let i = 0; i < root.view.length; ++i) {
            if (root.view[i].path === wasOn) { root.sel = i; break; }
          }
        }
        ...
        if (land) Qt.callLater(root.positionSel);
          ...
          // inside `root.rows = ...`, before this function returns — so
  ...
  function wheelTarget(bx, by) {
    ...
    if (!onActive)
      return root.paneViews[root.paneSide === 0 ? 1 : 0] === "grid"
        ? otherGrid : otherList;
  ...
  Item {
    ...
    Column {
            ...
            delegate: Row {
              ...
              Text {
                ...
                color: index === root.crumbList.length - 1
                  ? root.crumbInk : (crumbMa.containsMouse ? Zenon.cyan : Zenon.muted)
                font.family: "JetBrainsMono Nerd Font Propo"
      ...
      Row {
        ...
        ListView {
          ...
          // null rather than an empty list, so a hidden view holds no delegates
          ...
          delegate: EntryRow {
            ...
            // whose size or date changed re-evaluates this one binding instead
            // of being destroyed and built again — see the note on viewModel.
            required property string path
            required property int index
            readonly property var row: root.rowFor(path)
            width: list.width
            entry: row
            current: index === root.sel
            ticked: !!root.marked[path]
        ...
        Row {
          ...
          ListView {
            ...
            // rows divide the pane exactly — see the note on the list
            height: parent.height
            clip: true
          ...
          ListView {
            ...
            delegate: EntryRow {
              // identity from the model, the row from viewIndex — see viewModel
              required property string path
              required property int index
              readonly property var row: root.rowFor(path)
              width: midList.width
          ...
          Item {
            ...
            ListView {
              ...
              // rows divide the pane exactly, like every other listing — this
              // one was missed, so the miller preview went on cutting its last
              // row in half while the three columns beside it did not
              // GATED ON THE KIND, not merely hidden by it: previewRows now
              // carries an archive's tree as well, and those rows are not
              // entries — no path, no size, nothing an EntryRow can read. An
              // invisible ListView still builds its delegates, so a bare
        ...
        GridView {
          ...
          clip: true
          model: root.viewMode === "grid" ? viewModel : null
          boundsBehavior: Flickable.StopAtBounds
          ...
          delegate: Tile {
            id: tileItem
            // identity from the model, the row from viewIndex — see viewModel.
            // A tile holds a decoded Image, so surviving a refresh rather than
            // being rebuilt is worth more here than anywhere else in the window.
            required property string path
            required property int index
            readonly property var row: root.rowFor(path)
        ...
        DropArea {
          ...
          onDropped: (d) => {
            ...
            root.dropDir = "";
            root.dropUris(d.urls, d.proposedAction, into, dropHint, d.x, d.y);
          }
        }
    ...
    Item {
      ...
      ClippingRectangle {
        ...
        topLeftRadius: subCard.visible && !subCard.onRight ? 0 : 6
        bottomLeftRadius: subCard.visible && !subCard.onRight ? 0 : 6
        topRightRadius: subCard.visible && subCard.onRight ? 0 : 6
        bottomRightRadius: subCard.visible && subCard.onRight ? 0 : 6
      ...
      ClippingRectangle {
        ...
        topLeftRadius: subCard.onRight ? 0 : 6
        bottomLeftRadius: subCard.onRight ? 0 : 6
        topRightRadius: subCard.onRight ? 6 : 0
        bottomRightRadius: subCard.onRight ? 6 : 0
