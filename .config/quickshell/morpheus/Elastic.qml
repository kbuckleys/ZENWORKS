...
QtObject {
  ...
  // measured from here.
  function lo(v) { return v.originY || 0; }
  function hi(v) { return elastic.lo(v) + Math.max(0, v.contentHeight - v.height); }
  ...
  function letGo() {
    ...
    if (v && !settle.running && !anim.running) {
      v.contentY = Math.max(elastic.lo(v), Math.min(elastic.hi(v), v.contentY));
    }
  }
