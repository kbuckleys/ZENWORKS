...
PanelWindow {
  ...
  // dead halfway down with the picker gone. See OraclePopup.capturing, which
  // is the same rule written against a mask.
    ...
    ClippingRectangle {
      ...
      Column {
        ...
        Item {
          ...
          Text {
            anchors.centerIn: parent
            visible: popup.filtered.length === 0 && !Picasso.scanning
          ...
          Working {
            anchors.centerIn: parent
            running: Picasso.scanning && popup.filtered.length === 0
            ink: popup.dimColor
            dot: 5
            gap: 7
          }
