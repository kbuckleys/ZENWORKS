...
PanelWindow {
  ...
  readonly property color borderColor: "#339bbfbf"
  ...
  readonly property color exColor: "#eebebe"
      ...
      Repeater {
        ...
        Text {
          ...
          text: "<b><span style=\"color:" + popup.keyColor + ";\">" +
            Lexicon.escapeHtml(modelData[0]) + "</span></b> <span style=\"color:" +
            popup.dimColor + ";\">" + Lexicon.escapeHtml(modelData[1]) + "</span>"
  ...
  function drainWrite() {
    ...
    writeProc.command = ["sh", "-c",
      "printf '%s' " + Lexicon.shellQuote(job.content) + " > " + Lexicon.shellQuote(job.path)];
  ...
  function hints() {
    ...
    return popup.appMode === "dict"
      ? [["return", "define / history"], ["alt c", "clear"], ["tab", "translate"]]
      : [["return", "play"], ["shift return", "language"], ["alt c", "clear"],
  ...
  function buildDictRows(res) {
    ...
    for (const group of res.pos) {
      rows.push({ kind: "pos", text: Lexicon.escapeHtml(group.name) });
      for (const d of group.defs) {
        const label = d.label ? "(" + d.label + ") " : "";
        const lab = label
          ? "<span style=\"color:" + popup.dimColor + ";font-style:italic;\">" +
            Lexicon.escapeHtml(label) + "</span>"
          : "";
        rows.push({ kind: "def", text: lab + Lexicon.escapeHtml(d.def) });
        if (d.example)
          rows.push({ kind: "ex", text: Lexicon.escapeHtml(d.example) });
      }
    }
    if (res.synonyms && res.synonyms.length > 0) {
      rows.push({ kind: "syn", text: "<b>Synonyms:</b> " +
        Lexicon.escapeHtml(res.synonyms.join(", ")) });
    }
    return rows;
  }
  ...
  function prefetchDictAudio(file) {
    ...
    popup.detach("rm -f " + Lexicon.shellQuote(popup.dictAudioPath) +
      " " + Lexicon.shellQuote(popup.dictAudioPath + ".part") + " && " +
      "curl -sL --max-time 10 -A " + Lexicon.shellQuote(Lexicon.UA) + " " +
      Lexicon.shellQuote(url) + " -o " +
      Lexicon.shellQuote(popup.dictAudioPath + ".part") + " && mv -f " +
      Lexicon.shellQuote(popup.dictAudioPath + ".part") + " " +
      Lexicon.shellQuote(popup.dictAudioPath));
  }
  ...
  function confirmLanguage() {
    if (popup.langRows.length === 0) return;
    const lang = popup.langRows[Math.min(popup.sel, popup.langRows.length - 1)];
  ...
  function stopAudio() {
    ...
    if (popup.activePrefix !== "") {
      popup.detach("pkill -f " + Lexicon.shellQuote(popup.activePrefix) +
        " >/dev/null 2>&1");
    }
    popup.activePrefix = "";
  }
  ...
  function clampSel() {
    const len = popup.historyModel().length;
  ...
  function moveSel(delta) {
    const len = popup.view === "picker"
      ? popup.langRows.length
      : popup.historyModel().length;
    if (len === 0) return;
  ...
  Item {
    ...
    anchors {
      ...
      bottomMargin: popup.screen && popup.statusbar && popup.statusbar.screen &&
          ...
          Item {
            ...
            Column {
              ...
              Text {
                ...
                visible: popup.live !== null && !popup.live.error
                text: popup.live && !popup.live.error
                  ? "<b>" + Lexicon.escapeHtml(popup.live.translation) + "</b>" : ""
                color: popup.fgColor
              ...
              Text {
                ...
                text: popup.live && popup.live.roman
                  ? "<i>" + Lexicon.escapeHtml(popup.live.roman) + "</i>" : ""
                color: popup.exColor
                wrapMode: Text.WordWrap
                horizontalAlignment: Text.AlignHCenter
                textFormat: Text.RichText
                ...
                Row {
                  ...
                  Text {
                    text: popup.dictLoading
                      ? Lexicon.escapeHtml(popup.dictQuery) + " …"
                      : Lexicon.escapeHtml(popup.dictQuery)
                    color: popup.headColor
                    anchors.verticalCenter: parent.verticalCenter
                    font.family: "JetBrainsMono Nerd Font Propo"
      ...
      Item {
        ...
        Column {
          ...
          ListView {
            id: pickerList
            width: parent.width
            height: popup.listBodyH(popup.langRows.length)
            clip: true
            model: popup.langRows
  ...
  function calcHeight() {
    ...
    if (popup.view === "picker")
      return 72 + popup.listBodyH(popup.langRows.length);
