...
ShellRoot {
  ...
  // a click on the bare desktop is a click away from terminus' menus
  Connections {
    target: icarusDesktop
    function onDesktopPressed() { terminus.dismissMenus(); }
  }
  ...
  SocordiaDaemon { id: socordia; lockscreen: cerberus }
  CynosurePopup { id: cynosure; keepAwake: socordia.forceAwake; statusbar: bar; screen: root.focusedScreen; morphMode: root.morphedFor("cynosure") && root.morphOnPill; morphFade: root.layerFade; morphRadius: root.pillRadius }
  ...
  // what is waiting to be installed, out of the update count the way howler
  // comes out of the bell — see ceres/Ceres.qml for the checking itself
  CeresPanel { id: ceres; statusbar: bar; screen: root.focusedScreen; morphMode: root.morphedFor("ceres") && root.morphOnPill; morphFade: root.layerFade; answersAuth: !Ceres.windowShown }
  // and the full package manager, a real window like terminus — built off
  // the startup path, see CeresManager
  CeresManager { }
  // a remembered sudo password does not survive the screen locking
  Connections {
    target: cerberus
    function onLockedChanged() { if (cerberus.locked) Ceres.forgetPassword(); }
  }
  ...
  PanelWindow {
      ...
      //
      // THE RECTANGLE, NOT THE ITEM. `item: bg` maps bg through its
      // transform — and bg has one, the 22px lift the arrival plays — but
      // it re-reads that mapping only when bg's own geometry changes, never
      // when the transform moves. So the region was taken at the START of
      // the arrival, 22px below where the pill came to rest, and stayed
      // there until the first morph resized bg and forced a re-read: only
    ...
    //
    // It arrives WITH the pill: the intro fades bg in and lifts it 22px,
    // and a shadow that did neither sat finished on the desktop while the
    // pill was still rising into it. See LayerShadow's `fade`.
    LayerShadow {
      panel: bg
      cornerRadius: root.pillRadius
      fade: root.barIntro
      transform: Translate { y: pillLift.y }
    }
