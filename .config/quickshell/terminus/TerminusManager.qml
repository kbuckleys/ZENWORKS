...
Scope {
  ...
  //   - nothing prunes it. A folder that has gone simply never matches a
  //     row; the file is capped at the most recent `treeCap`, oldest first.
  //   - collapsing a folder removes only that folder, so what was open
  ...
  // first asks for one.
    ...
    IpcHandler {
      ...
      function spawn(path: string): string {
        ...
        // A hidden spare is re-aimed rather than a new window built — which
        // is the difference between a second window that is already there
        // and one that has to be compiled into existence. Sent home unless
        // told otherwise: a spare is a fresh window, not the last one's
        // place, and it is held until the new listing is in.
        const sp = mgr.spare();
        if (sp) {
          sp.goTo(path && path !== "" ? path : Paths.home());
          sp.revealWhenReady();
          return "window " + sp.winId;
        }
      ...
      function pick(multiple: string, directory: string, save: string,
        ...
        if (saving && path !== "") {
          ...
          // request follows a save of that same name into that same folder.
