...
// The next size up, wrapping back to the smallest — one button rather than
// two, because a note has four sizes and a stepper with an end is a button
...
// ── WRAPPING A SELECTION IN A TAG ────────────────────────────────────────
// TextEdit's font properties are document-wide, so bold-the-selection is not
// a property to set — it is an edit. The selection is replaced with itself
// inside the tag, which is what a rich-text editor does under the hood.
//
// Returns the html to insert, or null when there is nothing selected.
function wrapped(sel, tag) {
